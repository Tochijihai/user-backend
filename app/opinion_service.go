...
// PostUserComments - コメント取得API
func (s *OpinionService) GetUserComments(ctx context.Context, opinionId string) (openapi.ImplResponse, error) {
	// DynamoDBにコメントを保存する処理
	comments, err := s.db.GetComment(
		ctx,
		opinionId,
	)
	if err != nil {
